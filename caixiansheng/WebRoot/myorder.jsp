  ...
  <body>
    ...
    <s:if test="#session.myorder!=null">
		<div class="orderbox">
			...
			<table>
				<tr>
					<th>选择</th><th>订单号</th><th>商品名称</th><th>订单价格</th><th>订单状态</th><th>下单时间</th><th>收货地址</th>
				</tr>
				<s:iterator value="#session.myorder" var="od" id="ord">
					<tr>
						<td style="text-align: center;"><input class="checkone" name="chkName" type="checkbox" value="${ord.onumber}"></td>
						<td>${ord.onumber}</td>
						<td>${ord.pname}</td>
						<td>${ord.oprice}</td>
						<td>${ord.ostatus}</td>
			...
			<input type="submit" id="deletebutton" value="删除选中订单">
			</form>
		</div>
	</s:if>
	<s:else>
