...
define('utility',[],function () {
    ...
    var ui = {
        resoureUrl:{userInfo:'/home/userinfo'},
        getMessageType: function (msgtype, content) {
            ...
            if (msgtype == 4) {
                ...
                msgcontent = content.substr(0, 10);
        ...
        audioEvent: function () {
            ...
            pushhtml.push('<source src="' + window.im_static_baseUrl + '/sounds/Sent.wav" type="audio/wave"/>');
            pushhtml.push(' <source src="' + window.im_static_baseUrl + '/sounds/notification.mp3" type="audio/mpeg"/>');
            pushhtml.push('</audio>');
        ...
        changeIcon: function (number) {
            ...
            if (number > 0) {
                ...
                $(".hn-tbar-tab-im .tab-ico").css("background", "url(" + im_static_baseUrl + "/images/im/mes_icon.gif) 0px  center no-repeat");
            } else {
                $(".hn-tbar-tab-im .tab-text").text("在线咨询");
                $(".hn-tbar-tab-im .tab-ico").css("background", "#666 url(" + im_static_baseUrl + "/images/im/mes_icon.png) 7px  center no-repeat");
            }
        },
        getIsLogin: function () {
            var isLogin = true;
    ...
    var BaseVM= function () {
        ...
        self.GetStaticImg = function (src) {
            if (!src) {
                ...
                return window.im_static_baseUrl + '/' + pic;
            }
...
define('quickmsg',['lib/knockout-3.4', 'baseVM', 'utility', 'contacts'], function (ko, BaseVM, ui, contacts) {
    ...
    var QuickReplyVM = function (ChatWindow)
    {
        ...
        self.InitQuickReplyMsg = function (isTrue) {
            ...
            console.log('加载快捷回复..');
            ui.hnajax("/quick/GetList", null, "json", function (result) {
                ...
                if (result.responseData) {
                    ...
                    self.QuickReplyList(result.responseData);
                    $('.quick-reply').show();
                    setTimeout(function () {
                        $('.quick-list li').not('.cur').unbind('hover').hover(function () {
                            $(this).addClass('hover');
        ...
        self.Delete = function (data) {
            quickajax("/quick/DelQuick", { "id": data.id });
        }
        ...
        function quickajax(url, data) {
            ui.hnajax(url, data, "json", function (result) {
                ...
                if (result.responseStatus !=0) {
                    ui.showMessage(result.responseMsg);
                    return false;
                }
        ...
        self.Submit = function (data) {
            ...
            quickajax("/quick/UpdateQuick", { "id": data.id, "content": data.msgContent() });
            data.Editing(false);
        }
        self.Add = function () {
    ...
    function SendVM(ChatWindow) {
        ...
        function send(msg, msgType, isResend) {
            try {
                ...
                var currcontact = self.CurrContact();
                if (self.ishield()==1) {
                    ui.showMessage('你已屏蔽了与对方的会话.');
                    return false;
                }
                if (self.beIshield() == 1) {
                    ui.showMessage('对方已屏蔽你的会话.');
                    return false;
                }

                var currMsg = GetChatMsg(msg, msgType);
                var sendMsg = GetSendBody(msg, self.CurrContact().hnUserId, msgType);
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (result.errorCode == 0) {
                      ...
                      //  fileInfo.message.fileurl(result.data);
                    } else {
        ...
        self.SendFile = function (element, type) {
            ...
            reader.onload = (function () {
                return function (e) {
                    ...
                    uploadFile(im_static_baseUrl + '/api/v1.1/upload/upload-file', file, self.CurrContact().hnUserId);
        ...
        self.ResendMsg = function (data) {
            ...
            if (data.Head.MsgType == 1 || data.Head.MsgType == 3) {
                ...
                uploadFile(sendData.url, sendData, sendData.toUserId);
...
define('message',['lib/knockout-3.4', 'utility', 'fileMessage', 'cardInfo'], function (ko, ui) {
    ...
    function InitMsg(userId, ChatWindow) {
        ...
        self.InitMsg = function (isMore) {
            var params = { 'sid': userId, 'pageIndex': self.CurrPageIndex(), 'pageSize': self.CurrPageSize() };
            ui.hnajax("/chat/getList", params, "json", function (result) {
                ...
                var msgList = JSON.parse(result.responseData.items);
                self.LastHeight($(self.jqueryElementId() + " #ko_message-box")[0].scrollHeight);
                currList = msgList.reverse().concat(currList);
        ...
        self.IsCurrUserId = function (data) {
            ...
            if (data.Head.MsgType != 0) {

                try {

                    data.message = JSON.parse(data.Body.Content);
                  
                    if (data.Head.MsgType == 1) {
                        data.message.fileurl = ko.observable(data.message.fileurl);
                    }
                    if (data.Head.MsgType == 4) {
                        if (data.message.ShopName) {
                            data.message.shopName = data.message.ShopName;
                        }
                    }
                } catch (e) {
                    console.log('解析错误');
        ...
        self.GetUserImgSrc = function (data) {
            ...
            var curr = self.CurrContact();

            setting.currentPic = setting.currentPic || "images/headpic.png";
            data.Head.UserAvatar = (curr != null && curr.user_Pic) || "images/headpic.png";

            if (data.Head.UserAvatar.indexOf('ActivityTime:') != -1) {
                data.Head.UserAvatar = self.CurrContact().user_Pic;
            }
        ...
        //获取商家图标
        self.GetUserCert = function (data) {
            if (!data) {
                return '';
            }
            try {
        ...
        self.getorderUrl = function (message, data) {
            
            var link =  data.isCurrId() ? "buyer_" : "seller_";
            var host = window.currDomain == "cnhnkj.cn" ? "order." : "order.";
            host += window.currDomain;
            var url = "http://" + host + "/order/orderDetail/" + link+ message.id;
            return url;
        }
        self.getlinkUrl = function (message, type) {

            var link = type == 1 ? "/gongying/" : "/purchase/";
            var host = window.currDomain == "cnhnkj.cn" ? "test." : "www.";
            if (type == 0) {
                host = "buy.";
            }
            host += window.currDomain;
            var url = "http://" + host + link + message.id;
...
define('chatWindow',['lib/knockout-3.4', 'baseVM', 'utility', 'contacts', 'text!templates/chatWindow.html', 'sendTool', 'message'],
    function (ko, BaseVM, ui, contacts, html, sendTool, message) {
        ...
        function BindWindow(userId, dataKey, dataType) {
            ...
            ui.hnajax(ui.resoureUrl.userInfo, { uid: userId, currid: setting.currentId }, 'json', function (result) {
                ...
                chatwindow.ishield(result.ishield);
                chatwindow.beIshield(result.beIshield);
 
                chatwindow.CurrContact(result);
        ...
        function ChatWindowVM(userId, dataKey, dataType) {
            ...
            //是否屏蔽对方 1:屏蔽
            self.ishield = ko.observable(0);
            //是否被对方屏蔽  1:屏蔽
            self.beIshield = ko.observable(0);
            return self;
        }
        ...
        function btnClick() {
            ...
            ui.hnajax('/chat/openrecord', { jsonData: JSON.stringify(postData) }, 'json');
...
define('contacts',['lib/knockout-3.4', 'baseVM', 'utility', 'chatWindow', 'text!templates/contacts.html'], function (ko, BaseVM, ui, ChatWindow, html) {
    ...
    var ContactsVM = function () {
        ...
        self.UpdateMsgCount = function (receivId) {
            ...
            var param = { "uid": setting.currentId, "sid": receivId };
            ui.hnajax('/chat/UpdateMsgCnt', param, 'json');
        }
        //窗口列表
        ...
        self.Init = function () {
            ui.hnajax("/home/GetList", {pageIndex:1,pageSize:100}, "json",
                function (data) {
                    ...
                    var result = data.responseData.items;
        ...
        self.Remove = function (data) {
            ...
            if (confirm("确定删除此联系人吗?")) {
                ...
                ui.hnajax('/home/DelContact?' + Math.random(), {   'sid': data.userId }, 'json');
            }
            return false;
        }
...
require.config({
    ...
    baseUrl: im_static_baseUrl + "/scripts/pcim/im1.2/",
    paths: {
        ko: im_static_baseUrl + '/scripts/pcim/im1.2/lib/knockout-3.4',
        text: 'text',
    ...
    config: {
        "textremote": {
            remoteUrl: im_static_baseUrl + "/Scripts/pcim/im1.2/"
        },
        text: {
            useXhr: function (url, protocol, hostname, port) {
                return true;
                         ...
                         <div class="hn-toolbar-wrap">\
                           <div class="hn-toolbar">\
                             <div class="hn-toolbar-tabs">\
                              ...
                              <a target="_blank" href="http://crm2.qq.com/page/portalpage/wpa.php?uin=4000088688&aty=0&a=0&curl=&ty=1" rel="nofollow">\
